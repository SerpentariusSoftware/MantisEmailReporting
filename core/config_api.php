...
//		ERP_set_temporary_overwrite( 'subprojects_inherit_categories', OFF );
		ERP_set_temporary_overwrite( 'g_subprojects_inherit_categories', OFF );

//		$t_sel_value = (array) $p_sel_value;

		$t_sel_value = is_null($p_sel_value) ? 1 : (array)$p_sel_value;
		...
		foreach( $t_all_projects AS $t_project )
		{
			// Ensure keys exist to avoid warnings
		        $project_id = $t_project['id'] ?? null;
		        $project_name = $t_project['name'] ?? 'Unknown Project';

		        if ($project_id !== null) {
		            $t_projects_info[ (int) $project_id ] = $project_name;
		        }

...
- New: the Change Log page (and this file) now renders as actual Markdown -
  ...
  renders it with a newly vendored Markdown parser, `core/Markdown/Parsedown.php`
  (MIT-licensed). Safe mode is enabled so literal tag mentions in the prose
  (e.g. "`<form>`") render as visible text rather than being interpreted as
  actual markup
- New: redesigned "Manage Mailboxes" mailbox selection. The mailbox picker
  and its actions moved to the top of the page: pick a mailbox from the
  dropdown and the settings form below repopulates instantly via
  JavaScript (no page reload), including toggling the IMAP-only fields
  based on mailbox type. Add/Edit/Copy/Test/Complete test/Delete are now
  individual buttons that submit immediately, instead of a radio button
  selection followed by a separate generic submit button. Delete asks for
  confirmation before deleting; Add asks for confirmation if Description,
  Username or Password is left blank; Complete test asks for confirmation
  before running (replacing the old static warning note, since there's no
  longer an intermediate page state to show it on)
- Fixed the mailbox switcher's JavaScript not running at all: MantisBT's
  default Content-Security-Policy (script-src 'self', no nonce or
  'unsafe-inline' - see core/http_api.php's http_security_headers())
  silently blocks any inline <script> tag, which is how the switcher was
  first implemented. It now ships as a real external file
  (files/manage_mailbox.js, served via plugin_file()), with all
  server-rendered data passed through data-* attributes instead of
  PHP-interpolated JS literals
- Fixed the mailbox action buttons (Edit/Copy/Test/Complete test/Delete)
  staying invisible forever after being shown once: they had their own
  redundant "display:none" baked in server-side, in addition to their
  wrapping <span>, but the switcher only ever toggles the span's
  visibility - so the buttons' own inline style was never cleared
- Fixed the "Select mailbox" dropdown and its Add button visually
  swapping places regardless of source order: ace's admin theme sets
  ".inline{display:inline-block!important}", which beat the container's
  plain (non-!important) "display:flex" - once stuck at inline-block,
  Bootstrap's ".btn-toolbar .btn{float:left}" floated only the button
  (the <select> isn't a .btn), pulling it visually to the left. Both
  button rows now use plain styled containers instead of the
  "btn-toolbar"/"inline" theme classes
- After Add/Copy/Edit succeeds, "Manage Mailboxes" now redirects back
  with that mailbox selected and loaded, showing its Save/Copy/Test/
  Complete test/Delete buttons immediately - previously every submit
  redirected back to the blank "new mailbox" state regardless of action,
  so adding a mailbox left no obvious way to immediately continue editing
  or testing the one you'd just created
- New: "Manage Mailboxes" now starts on a minimal screen showing only the
  mailbox picker and an Add button; the settings form only appears once a
  mailbox is selected or Add is clicked. Clicking Add while the form is
  already showing something (an existing mailbox, or a blank one you've
  started filling in) now asks for confirmation before discarding it -
  except on that initial pristine screen, where there's nothing to lose
- New: Test and Complete test no longer submit the form or reload the
  page - they now run in the background (new pages/manage_mailbox_test.php
  endpoint) against whatever is currently in the fields, saved or not,
  and show the result inline. Previously, testing a brand new mailbox you
  hadn't saved yet (only possible after this update, since Test/Complete
  test now work before saving at all) would have discarded whatever you'd
  typed the moment the page reloaded to show the result. Complete test
  still asks for confirmation before running, same as before. The result
  is now also color-coded (green tint/border for success, red for
  failure), instead of always looking the same regardless of outcome
- New: while actively filling in a brand new, not-yet-saved mailbox, the
  Add button now relabels itself "Save" once it's genuinely about to
  persist that mailbox - it stayed labeled "Add" in every state before,
  even when it was the only way to actually save a new mailbox's fields
- New: the mailbox password field is no longer pre-filled with the stored
  (decoded) password. Leaving it blank when adding/editing/copying/testing a
  mailbox now means "keep the current password" instead of clearing it - this
  was required to enable the instant mailbox switcher above without exposing
  every configured mailbox's decoded password in the page at once, but is
  also a security improvement on its own: the stored password is no longer
  ever sent back to the browser at all
- Fixed a stray unclosed "<form>" tag on "Manage EmailReporting" (the
  mailbox-selector form at the bottom of the page) that should have been the
  closing "</form>" tag, leaving the page with invalid/unclosed markup
- New: added an exclusion list for the existing "Add users to issue monitoring
  list from Cc and To fields in mail header" feature (mail_add_users_from_cc_to).
  A new option, mail_monitor_exclude_addresses, lets you list addresses
  (prefix match, any domain - e.g. "helpdesk@" matches any domain) that should
  never be auto-added as monitors. Without this, the mailbox's own monitored
  address - which appears in the To field of every incoming email by
  definition - could get auto-subscribed to its own notifications if a Mantis
  user account shares that email, creating a feedback loop
- New: added mail_monitor_make_public (default ON), a new option that makes
  an issue public when a Cc/To recipient is auto-added as a monitor on it
  while it was private. MantisBT's notification filter
  (access_has_bug_level()) only exempts the reporter from the private-issue
  access check, not monitors, so a newly added Cc/To monitor would otherwise
  not actually receive the notifications they were just subscribed to unless
  the project gave them sufficient access - previously this had to be done
  by hand for every such issue

## Jul 2026 - EmailReporting-1.0.7 (this fork)
- Fixed accented characters (e.g. Hungarian o-double-acute/u-double-acute)
  in the email SUBJECT turning into '?' - process_header_encoding() trusted
  an RFC 2047 encoded-word's declared charset unconditionally, so a mail
  client/webmail gateway mislabeling the charset (e.g. tagging ISO-8859-2
  text as UTF-8) made mb_decode_mimeheader() silently substitute '?' for
  every unmappable character. Header decoding now validates the declared
  charset against the actual bytes and falls back to detection, mirroring
  the fix already applied to the body in 1.0.0. Raw, non-RFC2047 8-bit
  subjects (no "=?charset?...?=" wrapper at all) are now also charset
  detected instead of being passed through unconverted

## Jul 2026 - EmailReporting-1.0.6 (this fork)
- bug_report_mail.php now exits non-zero if any mailbox failed to process
  (bad login, connection refused, disabled mailbox, wrong project, etc.);
  previously such failures were only echoed into the job's output and never
  caused a non-zero exit, so bug_report_mail_cron.sh's failure alerting never
  fired for them - a broken mailbox could go unnoticed indefinitely
- Added ALERT_COOLDOWN_SECONDS to bug_report_mail_cron.sh (default 6 hours) so
  a persistent failure doesn't send one alert email per cron run; resets on
  the next successful run
- Fixed ERP_mailbox_api::process_mailbox() not resetting its internal result
  state per call, which could leak an earlier mailbox's error into a later,
  successful mailbox's reported result within the same run
- Dropped MantisBT 1.x compatibility (now requires MantisBT 2.0.0+); removed
  the legacy pre-2.0 layout code paths (html_page_top/html_page_bottom/
  print_bracket_link), which no longer exist in MantisBT 2.x
- Fixed a fatal error in manage_config_edit.php: it called the now-removed
  print_bracket_link() unconditionally (not behind the legacy-version check),
  which threw whenever the "Bug priority mapping" config field failed
  validation on save
- Fixed duplicate relationship history entries and a duplicate notification
  email sent when a reply links a new issue to an existing "master" bug -
  the plugin was manually repeating history logging/email that MantisBT's
  relationship_add() already performs internally
- Removed a dead argument to auth_generate_random_password() (the current
  MantisBT function takes no parameters and always returns a random nonce;
  the seed value was silently ignored)
- Fixed the database collation check on "Manage EmailReporting" flagging
  every single table/column as BAD regardless of actual collation. It
  compared against the literal string "utf8_", which stopped matching once
  MySQL/MariaDB started naming these collations utf8mb3_*/utf8mb4_* - so the
  check has been failing on every real-world server, including tables
  already on utf8mb4. Now correctly recognizes any utf8-family collation
  (utf8_, utf8mb3_, utf8mb4_) as OK
- Updated the "scheduled job" hint on "Manage EmailReporting" to recommend
  scripts/bug_report_mail_cron.sh (this fork's failure-alerting wrapper)
  instead of only showing the two raw ways to invoke bug_report_mail.php
  directly, and clarified that the second option is for a one-off manual
  test, not for scheduling

## Jul 2026 - EmailReporting-1.0.4 (this fork)
- Updated plugin author and website to Herczeg Szilard @ Serpentarius
  (https://serpentarius.hu/) across all language files and the plugin
  registration

## Jul 2026 - EmailReporting-1.0.3 (this fork)
- Added Hungarian translation (lang/strings_hungarian.txt), covering all 150
  plugin language strings

## Jul 2026 - EmailReporting-1.0.2 (this fork)
- Added a GPL-2.0 LICENSE file (matching the original plugin and MantisBT
  itself), and a README section pointing to it and noting the separate
  BSD/LGPL licenses of the vendored core_pear/ and Markdownify/ code

## Jul 2026 - EmailReporting-1.0.1 (this fork)
- Added scripts/patch_mantis_html_email.sh, an optional utility that patches
  MantisBT core (not this plugin) to send outgoing notification emails as HTML
  instead of plain text. Idempotent, takes timestamped backups. Written against
  and only tested with MantisBT 2.28.4, where email sending was restructured;
  not tested against other versions, and needs re-applying after a core upgrade
- Documented the new utility script, setup steps and full current feature set in
  the README
- Removed stray closing "?>" PHP tags from the language files

## Jul 2026 - EmailReporting-1.0.0 (this fork)
- PHP 8.0-8.4 compatibility: fixed a fatal parse error in the HTML-to-Markdown
  converter (duplicate "static $indent" declaration) that broke plugin load
  entirely on PHP 8.3+
- PHP 8.0-8.4 compatibility: replaced removed create_function()/each() calls in
  the vendored PEAR libraries (SASL SCRAM auth, PEAR destructor cleanup) with
  their modern equivalents
- PHP 8.0-8.4 compatibility: declared several dynamic properties across the
  vendored PEAR/Markdownify/simple_html_dom code to silence PHP 8.2+ deprecation
  warnings, and guarded a handful of trim(null)-style calls
- Dropped support for PHP versions older than 8.0
- Fixed a charset-detection bug that could turn accented characters (e.g.
  Hungarian o-double-acute/u-double-acute, or other Central/Western European
  letters) into '?', or in the worst case silently drop the entire email
  body/subject
- Added "Cut off quoted reply/history text" option (mail_strip_quoted_lines /
  mail_strip_quoted_lines_min_lines), which truncates the description/note once
  a run of consecutive '>' quoted lines is found, regardless of mail client or
  language
- New issues created in a private project now inherit that project's visibility
  (VS_PRIVATE) instead of always using the site default
- Added scripts/bug_report_mail_cron.sh, an optional cron wrapper that emails an
  alert (via sendmail) if bug_report_mail.php fails or times out, with a -s flag
  to also notify on success for testing. Server-specific settings live in a
  gitignored bug_report_mail_cron.env (see bug_report_mail_cron.env.example)
